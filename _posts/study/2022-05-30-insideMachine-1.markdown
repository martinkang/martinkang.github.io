...
![calculator-model](/assets/img/study-img/inside-machine/calculator.png ){:class="lazyload" .post-img .img-w400 .img-center}
*계산기 모델*
...
![calculate](/assets/img/study-img/inside-machine/calculate.png ){:class="lazyload" .post-img .img-w200 .img-center}
...
- 컴퓨터가 읽기-수정-쓰기를 수행하기 위해서는 다음 3가지 기본 요소가 필요하다.
  - 저장소
    - 컴퓨터가 데이터를 Read, Write 할때 이 데이터를 저장된/저장할 장소.
  - 산술 연산 장치 (ALU : Artimetic Logic Unit)
    - 저장소에서 읽어온 데이터와 코드의 명령어를 가지고 산술 연산을 수행하는 장치.
  - 버스 ( Bus )
    - ALU 와 저장소 사이에서 데이터를 전송하는 통로.

![calculator-model](/assets/img/study-img/inside-machine/alu-storage.png ){:class="lazyload" .post-img .img-w400 .img-center}
...
![alu-storage-register](/assets/img/study-img/inside-machine/alu-storage-register.png ){:class="lazyload" .post-img .img-w400 .img-center}
*레지스터가 결합된 문서 관리원 모델*
...
***메인 메모리와는 실제로 별개로 존재하여 메모리 버스를 통해 CPU 와 연결***된다.  
...
이를 ALU 를 통해 계산을 한뒤 저장을 한다.
  - 모든 프로그램은 결국 데이터를 가져와서 명령어에 따라 처리하고 이를 다시 저장하는 과정의 반복이다.
...
![program-ex](/assets/img/study-img/inside-machine/program-ex.png ){:class="lazyload" .post-img .img-w550 .img-center}
*간단한 덧셈 프로그램의 메모리 예시*
...
## 직접값
- 레지스터 대신 직접 사용하는값을 사용하는 방식.
  - A 에 있는 값을 2 증가시키기 위해 2 가 저장된 메모리 셀에서 데이터를 레지스터 B 에 읽어온 후 
    덧셈을 하는 대신 A 에 2 를 직접 더할 수 있다.
...
![program-ex](/assets/img/study-img/inside-machine/program-ex-imm.png ){:class="lazyload" .post-img .img-w550 .img-center}
...
- #D : 메모리 주소 저장을 위한 레지스터, 데이터 세그먼트의 기본 주소를 가지고 있음.
  ...
  ![program-ex](/assets/img/study-img/inside-machine/program-ex-rel.png ){:class="lazyload" .post-img .img-w550 .img-center}
