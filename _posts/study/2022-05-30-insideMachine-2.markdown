...
- 우리는 위의 모델을 이용하여 레지스터가 8개가 있다고 가정하고 프로그램을 작성하면 된다.
...
약간만 수정하면 대부분 마이크로 프로세서가 프로그램을 실행하는 과정을 설명할 수 있고 이를 페치 - 수행 루프 또는 페치 - 수행 싸이클이라 한다.
클럭
페치 - 수행 루프의 3 단계 과정을 처리하는 단위
페치 - 수행 루프는 1 클럭에 처리된다.
...
명령어
 jump #목적주소
프로세서는 단순히 제어 장치에 있는 프로그램 카운터의 값을분기 명령어의 목적 주소로 바꿔준다.
프로세서는 다음 클럭에서 프로그램 카운터가 가르키는 주소 ( 분기 명령어의 목적 주소 ) 에서 명령어를 페치하게 된다.
조건 분기
특정 조건이 만족될 때에만 분기한다.
프로세서 상태 워드( Processor Status Word: PSW )
산술 명령어가 수행될 때면, 연산 결과에 따라 다양한 형태의 정보가 PSW 에 저장
조건 분기를 실행하기 위해 PSW 에 저장되어 있는 값을 통해 분기 명령어의 조건을 확인.
ex ) jumpz #106 
PSW 값을 확인하여 값이 0 이면 메모리 주소 #106 에 저장된 명령어로 분기하고, 그렇지 않은 경우 프로그램 카운터의 주소값으로 분기.
분기 명령어를 이용한 특수 로드
일반적인 로드 명령어와 마찬가지로, 분기 명령어는 레지스터를 사용해서 목적 주소를 지정 가능하다.
이 기법을 사용하여 프로그램을 임의의 주소에 저장 가능하고, 프로그래머는 메모리를 관리하는 운영체제가 해당 코드의 시작 주소를 어디에 저장해두었는지만 알면 된다.
jump #(C + 30 )  코드 세그먼트 ( C 레지스터 )의 시작에서 30 바이트 떨어진 주소로 명령어 분기.


분기 명령어와 레이블
실제 프로그램에서 분기문의 목적 주소로 직젒값이나 상대 주소를 쓰는 경우는 드물고, 레이블을 지정하여 레이블을 분기 명령어로 사용한다.
ex ) 분기 성공시 jumpz 아래의 add 는 수행하지 않는다.
sub A, B, A
jumpz LBL1
add A, 15, a
LBL1 : add A, B, B
...
- 이 BIOS 는 대개 마더보드상의 ROM( Read Only Memory ) 에 저장된다.
- 바이오스는 주변 장치에 대한 기본적인 작동을 테스트 한 후, 무조건 분기를 한다.
- 이 바이오스의 목적 주소는 부트로더의 시작 주소이다.
- 바이오스는 컴퓨터의 제어 권한을 부트로더에 넘겨준다.
